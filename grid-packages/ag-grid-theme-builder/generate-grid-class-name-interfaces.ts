const main = async () => {
  ...
  const emit = async () => {
    ...
    for (const type of Object.keys(result).sort()) {
      const classes = additionalClasses[type] || [];
    ...
    await fs.writeFile(dstFile, beforeMatter + content + afterMatter, 'utf8');
  };

  await scanPackages('../../grid-community-modules');
  await scanPackages('../../grid-enterprise-modules');

  for (const coreClass of coreClassNames.values()) {
    result['Core'].add(coreClass);
  }

  for (const module of Object.keys(additionalClasses)) {
    if (!result[module]) {
      throw new Error(`Invalid module for additional classes: "${module}"`);
    }
  }
...
const additionalClasses: Record<string, string[]> = {
  Core: [
